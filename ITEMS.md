...
| 16 | Results-save hardening + environment provenance | **validated 2026-07-25** on run 3 and both same-day batches: `environment` block and `calibration_at_submit.json` present. Remaining: `git.dirty:true` on every `--hardware` run is benign (crash-safety writes precede provenance capture) but the structural fix — capture provenance before submission — is still open | G15, G16 | env-block template: results/hardware-scaling/preregistration.json |
