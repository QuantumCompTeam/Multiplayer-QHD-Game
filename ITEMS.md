...
| 10 | T9 learning rule | **blocked on Aasa — cannot be closed from this side**, but the sign-off is now evidence-backed rather than abstract. The rule was tested on the axis the caveat named: **simultaneous BR reproduces all three verdicts** (`docs/findings/2026-07-26-t9-rule-sensitivity.md`, 2026-07-26) — p=0 means agree to 0.15%, p=0.05 to 1e-4, and p=0.02 fails to settle under BOTH rules with the simultaneous residual flat at ~380x the convergence bar, so the non-convergence is a property of the noisy best-response map, not of move order. Fictitious play / regret matching still untested; caveat 2 (W N=5, ring N=5) still open. Author metadata is now in the paper (aasasingh2005@gmail.com). Until sign-off the fairness paragraph stays hedged as exploratory, which is its current wording | — | TODOS.md, "T9 adaptation & fairness pilot"; docs/findings/2026-07-05-t9-adaptation-fairness.md |
