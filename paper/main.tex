...
zero-noise extrapolation. For the registration-source execution, a single
...
CZ-exponential baseline is the lowest-scoring of five registered candidates in
both same-recorded-calibration executions, but it misses $N{=}5$ outside the
shared comparison yardstick in both; the ranking is not yet a physical scaling
law. The mean-payoff observable
...
The CZ-exponential baseline is the
lowest-scoring of the five registered candidates in both
same-recorded-calibration executions. It nevertheless
...
that spare the mean concentrate loss on individual players. In one exploratory
...
ring $N{=}5$ systems. Third, the CZ-exponential baseline is the lowest-scoring
registered candidate in both
scaling executions, but its $N{=}5$ misses and the shared ranking yardstick do
not establish a physical scaling law; cross-day executions and model-specific
predictive uncertainties remain necessary.
% results/hardware-scaling/repeat-judgments.json
...
not cross-day uncertainty; and the adaptation evidence is limited to one
exploratory W $N{=}4$ finite-round pilot using a provisional rule. Within those
