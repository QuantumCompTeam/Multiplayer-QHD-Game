...
\IEEEauthorblockA{VIT Vellore\\aasasingh2005@gmail.com}
}
...
\begin{figure*}[t]
  ...
  fits. The ideal, device-model and depolarizing curves are the
  ...
  deviation across the two calibration epochs --- a genuine cross-calibration
  estimate, unlike the within-epoch spread reported in earlier drafts --- and
  still exclude the within-run multinomial and weighted-fit standard errors,
...
--- of the five registered candidates in all four executions, including both
...
at unchanged mean welfare --- reported as a direction, not a magnitude, since
those endpoints are non-convergent snapshots. No belief-based rule has been
...
topology-optimized under noise. The chain-matched scaling series uses
$[59,75,74,73,79]$; the separate $N{=}3$ validation used $[136,143,142]$ and the
preceding recorded calibration date. Cross-calibration uncertainty now rests on
two chain-matched epochs (three distinct calibrations exist, but one ran on a
different physical chain and is excluded from the aggregate), and the
epoch-to-epoch variation in the $N{=}5$ deficit is of order the deficit itself.
...
finite-round, though its two self-raised caveats --- move order, and coverage
beyond W $N{=}4$ --- have both been tested without changing the verdict; and
