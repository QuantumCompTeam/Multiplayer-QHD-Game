...
%   pdflatex -interaction=nonstopmode -halt-on-error -output-directory=.qhd-build-check qhd.tex
%   pdflatex -interaction=nonstopmode -halt-on-error -output-directory=.qhd-build-check qhd.tex
% For Overleaf, upload the submission source ZIP and select qhd.tex.
% Layout follows the supplied FQCNN_journal.pdf: IEEEtran conference-style
% letter paper, 10pt two-column body, author blocks and black headings.
...
%
% STRUCTURE NOTE (2026-09-08): I Introduction / II Related Work /
% III Evaluation / IV Framework / V Methods / VI Results / VII Discussion /
% VIII Technical Differentiation / availability / IX Conclusion.
\documentclass[conference,letterpaper,10pt]{IEEEtran}
\usepackage[nocompress]{cite} % give every citation number its own clickable target
...
\setlength{\parskip}{0.35\baselineskip} % visible separation between paragraphs
\graphicspath{{figs/}}
...
\begin{enumerate}
  ...
  \item \textbf{Reproducible pipeline.} A versioned research harness with
...
reviews related work, and Section~\ref{sec:evaluation} defines the evaluation
questions and evidence boundaries. Section~\ref{sec:model} develops the
mathematical framework, followed by computation and validation methods in
Section~\ref{sec:methods}. Section~\ref{sec:results} reports simulation and
hardware results. Section~\ref{sec:discussion} discusses their interpretation,
scalability, and limitations. Section~\ref{sec:novelty} states the technical
differentiation, followed by data and code availability and the conclusion
...
Benjamin and Hayden~\cite{benjamin2001} give a multiplayer quantum-game
construction, including minority-game and Prisoner's-Dilemma examples.
We adopt its circuit framework; the shared-resource allocation payoff in
Sec.~\ref{sec:model} is the separately specified model studied here.
Chappell, Iqbal, and Abbott~\cite{chappell2012}
...
% =============================================================================
\input{evaluation-boundaries.tex}
...
can move the deviation incentives characterized in Sec.~\ref{sec:incentive-boundary}
while leaving the cooperative payoff exactly unchanged.

\input{incentive-boundary.tex}
% market interpretation: README §10

% =============================================================================
\input{validation-methods.tex}

\section{Experimental Results and Analysis}\label{sec:results}

\subsection{Simulation Landscape}\label{sec:sim}

\subsubsection{Scaling and Topology at Zero Noise}
...
\subsubsection{Entanglement-Angle Dependence}
...
\subsubsection{Player Symmetry Breaking in the Star Topology}
...
$N=4\ldots6$. Labels show gaps to two decimal places, including the
numerically zero hub gap at $N{=}4$; green bars show the per-player mean. Data:
\texttt{results/n-scaling-advantage/2026-07-19T0901Z}.}
\label{fig:star}
\end{figure}

\subsubsection{Noise Robustness: Two Criteria Disagree}\label{sec:noise}
...
\paragraph{Restricted-menu equilibrium criterion}
...
\paragraph{Circuit-relative payoff-gap magnitude criterion}
...
\subsubsection{Position-Locked Unfairness}
...
\subsubsection{The Finite-Round Adaptation Pilot}
...
% =============================================================================
\input{journal-results.tex}

\subsection{Hardware Validation on IBM Heron r2}\label{sec:hardware}

\subsubsection{Single-Point Validation at $N=3$}
...
\subsubsection{Scaling at $N=3,4,5$ with Error Mitigation}
...
\emph{Calibration grouping and inclusion.}
The aggregate includes three executions on $[59,75,74,73,79]$, with
4096 shots per circuit, spanning two provider calibration stamps:
2026-07-16 08:30:13 +05:30 (two executions) and
2026-07-26 13:29:01 +05:30 (one execution).
Executions sharing a stamp are averaged before computing the spread so that
one calibration is not counted twice. A fourth execution on
$[20,21,22,23,24]$ is excluded because pooling it would confound calibration
drift with a change of physical qubits. Table~\ref{tab:scaling} and
Fig.~\ref{fig:scaling-zne} show the registration-source execution alone.

\emph{Models and uncertainty.}
The ideal, device-model and depolarizing curves come from the registration
anchor, not averages of refitted models. The depolarizing parameter
$p_{\mathrm{eff}}=0.0018$ is fitted only at $N=3$; its $N=4,5$ values are
predictions. In Figs.~\ref{fig:scaling} and~\ref{fig:scaling-population},
error bars are the sample standard deviation across the two calibration
epochs. They exclude within-run multinomial and weighted-fit standard errors,
and are a lower bound on total uncertainty. Figure~\ref{fig:scaling-zne}
instead shows within-run payoff standard errors for the registration-source
fit. Among the plotted multiplayer cases, $(Q,\ldots,Q)$ passes the restricted
pure-Nash check only at $N=3$; $N=4,5$ measure a cooperative protocol,
not an equilibrium.

The all-zero target population in Fig.~\ref{fig:scaling-population} decays
faster than the advantage: the mean-payoff observable is first-order
insensitive to single bit-flips from $|0\ldots0\rangle$, since a one-Hawk
outcome retains mean payoff $V/N$. The observable mechanism is developed
in Sec.~\ref{sec:outlives}.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{hardware_scaling_advantage.pdf}
  \caption{Cooperative-profile payoff advantage on ibm\_fez at $N=3,4,5$,
  relative to the analytic noiseless classical-Nash baseline, comparing raw
  and mitigated-plus-ZNE measurements with ideal and noise-model curves.
  Error bars show the sample standard deviation across two calibration epochs
  and are a lower bound on total uncertainty; depolarizing values at $N=4,5$
  are predictions from the $N=3$ fit.
  The fixed profile passes the restricted pure-Nash check only at $N=3$;
  $N=4,5$ measure a cooperative protocol, not an equilibrium.}
  \label{fig:scaling}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{hardware_scaling_zne.pdf}
  \caption{Zero-noise extrapolation of the readout-mitigated mean payoff in
  the registration-source execution at $N=3,4,5$.
  Points at CZ fold factors $1,3,5$ carry within-run payoff standard errors;
  weighted linear fits extrapolate to zero fold factor (stars).
  The $N=4,5$ profiles are cooperative protocols, not restricted pure-Nash
  equilibria.}
  \label{fig:scaling-zne}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{hardware_scaling_population.pdf}
  \caption{All-zero output probability on ibm\_fez at $N=3,4,5$, compared
  with the device noise-model prediction and ideal target.
  Measured points average calibration epochs; error bars are the cross-epoch
  sample standard deviation and a lower bound on total uncertainty.
  These are fixed-profile measurements, with the restricted pure-Nash property
  holding only at $N=3$.}
  \label{fig:scaling-population}
\end{figure}
...
That ranking is specific to $N\le 5$: in the registered $N=6,7$
...
\subsubsection{Extension to $N=6,7$}\label{sec:n67}
...
$0.784$: the advantage survives to seven players at $98\%$ of ideal
...
$N=4,5,6$ but the depolarizing model is closer at $N=7$. The
...
\subsubsection{Topology, Equilibrium and Fairness on Device}\label{sec:hwtopology}
...
\paragraph{Registered Hawk-deviation tests on hardware}
...
The registered null therefore holds: the disadvantaged position is
...
\input{phase-hardware-results.tex}
...
\href{https://github.com/QuantumCompTeam/Multiplayer-QHD-Game}{Multiplayer-QHD-Game}
repository. A supplementary code-and-data archive accompanies the submission
materials as a fixed, hashed snapshot for review.
The \texttt{results/phase-validation} directories contain the
committed manifests, serialized circuits, raw counts, calibration snapshots,
and provider metrics for the new runs. The \texttt{results/journal-strengthening}
directory records compact-evaluation and retrospective-sensitivity outputs.
The claim ledger and revision log map manuscript statements to their evidence.
Historical simulation dependencies and the current hardware environment are
recorded separately; replay of a frozen execution uses its recorded code
revision as well as its dependency versions.
