...
\markboth
{Raghu and Bhui: Entanglement Topology and Noise Determine Quantum Advantage in $N$-Player Trading Games}
{Raghu and Bhui: Entanglement Topology and Noise Determine Quantum Advantage in $N$-Player Trading Games}

\begin{abstract}
% Real numbers: results/hardware-scaling/2026-07-16T074134Z/, README §9.
% NOTE: condensed 2026-07-26 from the long-form abstract. Nothing was
% strengthened; the dropped sentences (per-sigma model residuals, the
% CZ-exponential ranking detail, the W per-gate-retention figure, and the
% adaptation pilot) all survive in full, with their original hedges, in
% Secs. IV-D, V-C and V-D. The prior version is in the pre-restructure main.tex.
We extend the two-player quantum Hawk--Dove trading game of Khan et al.\ to
$N$ players under five entanglement topologies (GHZ, W, ring, star, fully
connected) using the Eisert--Wilkens--Lewenstein (EWL) protocol, and map how
topology, player count, and depolarizing noise jointly determine quantum
advantage. For the GHZ entangler the fixed cooperative profile pays exactly
$V/N$ and passes a restricted $\{D,H,Q_N\}$ deviation check at $N{=}2,3$ (not an equilibrium test against arbitrary SU(2) gates), while the other
topologies are evaluated cell by cell. Under noise the restricted-menu
equilibrium and circuit-relative payoff-gap criteria \emph{disagree}, and
noise breaks player symmetry into position-locked per-player losses that the
mean hides. On an IBM Heron r2 processor (ibm\_fez) the measured advantage
retains $98$--$99\%$ of the ideal $3/N$ at $N=3,4,5$ after readout mitigation
and zero-noise extrapolation, and $98.1\%$ at $N{=}7$ with state fidelity
already down to $0.78$. A registered 49-pub multi-topology batch puts the
topology, equilibrium and fairness claims on device: all twelve runnable
topology$\times N$ cells retain $97.9$--$99.6\%$ of their own ideal advantage,
no unilateral Hawk deviation pays at GHZ $N{=}3$, and a wiring permutation
that reassigns players to different physical qubits leaves the star $N{=}4$
hub's disadvantage intact, so position-locked unfairness is a property of the
entanglement topology rather than of qubit allocation. Registered
one-parameter noise models nonetheless fail their tests at $N{=}6,7$ and their
ranking does not extrapolate. The mean-payoff observable is first-order
insensitive to single bit-flips, so the advantage decays several times slower than state fidelity, a structural robustness with direct consequences for
...
institution. For market mechanisms such as carbon-trading schemes, order-book architectures, and DeFi protocols, this suggests replacing a regulatory cost
...
entangles the player register before any strategy is applied, so the
correlation structure of the shared state, rather than an external
institution, determines what is self-enforcing. The celebrated two-player
result is a quantum Nash equilibrium that is simultaneously symmetric and
Pareto-optimal, with maximal entanglement reached at
$\gamma=\pi/2$~\cite{benjamin2001,chappell2012}. The protocol is the base
...
Khan et al.~\cite{khan2025} demonstrated a quantum Nash-equilibrium advantage
for a two-player trading game on ion-trap hardware. Two players, however,
admit exactly one entanglement geometry, so the topology axis does not exist
at $N{=}2$: the question of which correlation graph a market designer should
...
entanglement angle $\gamma$ measures how strongly the register is correlated
before the players act: every family we study satisfies $J_T(0)=I$, the identity
on all $N$ qubits, so
$\gamma=0$ returns the classical game exactly, and $\gamma=\pi/2$ is the
maximally entangled setting~\cite{chappell2012} (that work writes the same angle
as $\theta$). The subscript $T$ is the only thing left undefined here; the five
...
outcome. It is not state fidelity: it does not determine the remaining
populations or any coherence of the output state, and it is distinct from the
payoff entry $P_j(x)$.
...
\subsection{Architectural Interpretation}\label{sec:archinterp}
...
network, W a partial-information market, and fully connected a dense interbank
market. Under this reading the entanglement graph is not an implementation
detail but the mechanism's connectivity specification, and the results that
follow measure what each specification costs and buys.
...
mechanism behind the scaling section's central observation, that the advantage decays several times slower than the state fidelity, and it is why the
...
\subsubsection{The restricted-menu equilibrium holds on hardware}
...
% market interpretation: README §10
The advantage scales further than the state does: it survives to seven players
at $98\%$ of ideal on a chain whose ground-state probability has already fallen
to $0.78$, so the binding constraint on this family of mechanisms is the
two-qubit budget rather than the coherence of the shared state. The
two-criteria dichotomy of Sec.~\ref{sec:noise} yields a concrete design rule:
choose the topology by whether the mechanism needs the \emph{equilibrium} or
the \emph{payoff}. Restricted-menu equilibrium results are cell-specific: at
...
makes it the choice when two-qubit gates, not entanglement structure, are the binding constraint.

Two applications illustrate the analogy. In carbon markets (an $N$-player
Hawk--Dove between emission reduction and allowance purchase), the topology
that maximises cooperative payoff at a given noise budget is the optimal
architecture for a quantum-enhanced trading mechanism. In DeFi, where
front-running (MEV) is an $N$-player Hawk--Dove that destroys value,
entangling wallet interactions via a smart-contract-controlled quantum channel
could enforce cooperative outcomes without a trusted intermediary; the entanglement graph suggests a possible contract-interaction architecture.

\subsection{Limitations and Future Work}
Discrete strategy set $\{D,H,Q_N\}$ (mixed/continuous deviations are future
work); the fixed GHZ-derived $Q_N$ is not topology-optimized under noise. The
chain-matched scaling series uses $[59,75,74,73,79]$; the separate $N{=}3$
validation used $[136,143,142]$ and the preceding recorded calibration date.
Cross-calibration uncertainty now rests on two chain-matched epochs (three
distinct calibrations exist, but one ran on a different physical chain and is
excluded from the aggregate), and the epoch-to-epoch variation in the $N{=}5$
deficit is of order the deficit itself. Model-specific predictive
uncertainties remain needed, and no repeat of the $N=3\ldots7$ batch has been
run, so the $N{=}7$ retention anomaly of Sec.~\ref{sec:n67} stays unresolved.
The T9 adaptation evidence uses one provisional finite-round rule; it has been
checked against a simultaneous-update rule and extended to W $N{=}5$ and ring
$N{=}5$, but belief-based rules such as fictitious play remain untested.

% =============================================================================
\section{Novelty and Technical Differentiation}\label{sec:novelty}

The proposed framework departs from previous $N$-player quantum-game work on
four precise technical fronts that together make up the main contributions of
this paper.

\subsection{Topology as a design variable}
Prior $N$-player hardware work characterises entanglement operators
generically~\cite{varsamis2025}, and the GHZ-versus-W comparison of Chappell
et al.~\cite{chappell2012} was carried out in an EPR setting without a
financial payoff structure or a hardware validation. Here the game is fixed
and the correlation graph is the variable: we report the complete
$(N,\text{topology})$ circuit-relative payoff-gap map over five topologies for
$N=2$--$6$, and take twelve of those cells to device.

\subsection{Two robustness criteria that disagree}
Robustness in this setting is not one quantity. The restricted-menu
equilibrium criterion and the circuit-relative payoff-gap magnitude criterion
order the same topologies differently under the same depolarizing sweep
(Sec.~\ref{sec:noise}), so a mechanism designer must state which one the
mechanism needs before a topology can be called robust. We report both and
keep their baselines separate rather than reconciling them into a single
score.

\subsection{Position-locked unfairness}
Noise does not degrade players equally, and the disadvantaged position is
carried neither by the player label nor by the physical qubit. Simulation
shows the per-player payoff vector permuting rigidly with the qubit-to-player
wiring, and a registered hardware wiring permutation on star $N{=}4$ moves the
measured vector by at most $0.035$ while leaving the hub--leaf split intact.
What remains is the graph position, which makes the per-player floor, not the mean, the relevant diagnostic for mechanism design.

\subsection{Registered, falsifiable hardware protocol}
Predictions were written and committed before submission, including two
competing one-parameter noise models fitted only at $N{=}3$, and they are
reported as they landed: both fail their registered tests at $N=6,7$, the
per-cell absolute-advantage predictions failed outright, and the $N{=}7$
retention anomaly is reported as unexplained. The structural claims are
within-job differentials and do not depend on those failures. The
supporting harness enforces circuit-identity assertions, a noiseless dry-run,
and a device-model dress rehearsal before any submission, with crash-safe job
recovery.
...
At $N{=}4,5$ the same profile ceases to be an equilibrium, since a unilateral Hawk deviation profits, yet its cooperative payoff retains a large,
...
The dichotomy, equilibrium at $N{=}3$ and cooperative profile beyond, is real, and it is a feature of the game, not a defect of the experiment. The
...
disadvantage intact. In an exploratory
finite-round adaptation pilot, the provisional rule reached the 50-round cap
without convergence at $p{=}0$, and no tested setting produced a beneficial
converged fairness repair, a verdict unchanged under simultaneous updates and
reproduced at W $N{=}5$, while ring $N{=}5$ fails to converge at any tested
noise level and spreads a fair baseline apart instead of repairing it.
Third, the CZ-exponential baseline carries the best
registered score in all three
scaling executions at $N\le5$, but its $N{=}5$ misses and the shared ranking
yardstick do not establish a physical scaling law, and in the registered $N{=}6,7$ extension it loses to the depolarizing model at $N{=}7$ while both
...
the registered target of three to five distinct calibrations is met at three,
but only two of those are chain-matched, so the plotted cross-calibration
uncertainty rests on two epochs and the $N{=}5$ deficit magnitude varies by a
factor of two across them; the adaptation evidence remains exploratory and
finite-round, though its two self-raised caveats, move order and coverage beyond W $N{=}4$, have both been tested without changing the verdict; and
...
bounds the design implication is concrete: entanglement topology is a market-design variable whose noise cost is budgeted per
